...
{
    /// <summary>
    /// Every pixel the mod's windows draw comes from the running game: the TMP font off the HUD
    /// and the chrome sprites out of the UIAtlas. Nothing is shipped on disk.
    ///
    ...
    internal static class UiTheme
    {
        ...
        /// <summary>
        /// The same, with a black outline and a shadow. For text drawn straight over the world or
        /// a 3D picture, where the background is whatever the camera happens to be looking at.
        /// </summary>
        ...
        /// <summary>The black edge under white text over the world. Wide enough to carry a sky.</summary>
        ...
        public static bool Ensure()
        {
            ...
            FontMaterial = OwnTextMaterial(source, "MyValheimModText", NoColour, 0f, false);
            FontOutlined = OwnTextMaterial(source, "MyValheimModTextOutlined", Color.black, OutlineWidth, true);

            var atlas = Resources.FindObjectsOfTypeAll<SpriteAtlas>().FirstOrDefault(a => a.name == "UIAtlas");
            if (atlas == null)
            {
                MyValheimModPlugin.Log.LogWarning("UIAtlas not found: the mod's windows will draw without chrome.");
            }
            ...
            MyValheimModPlugin.Log.LogInfo(
                $"ui theme ready | font={(Font != null ? Font.name : "none")} | sprites={Copies.Count}/9");
            return Font != null;
        }
        ...
        /// <summary>
        /// The mod's own copies of the HUD's text material.
        ///
        ...
        /// world or a 3D picture, where the background can be as light as the sky.</item>
        /// </list>
        ...
        private static Sprite Take(SpriteAtlas atlas, string name)
        {
            ...
            if (sprite == null)
            {
                MyValheimModPlugin.Log.LogWarning($"UIAtlas has no sprite '{name}'.");
                return null;
            }
