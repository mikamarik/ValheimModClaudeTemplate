...
namespace MyValheimMod.Ui
{
    ...
    /// <c>&lt;sprite="gamepad_glyphs" name="button_a"&gt;</c>. A window that wants them as pictures
    /// it can size and line up next to a key cap gets a Sprite here instead of a tag.
    /// Inside a TMP text the tag itself is simpler: <c>Localization.GetBoundKeyString</c> hands out
    /// the game's own tag for a button, in the family and layout the player has.
    ///
    ...
    internal static class PadGlyphs
    {
        ...
        private static TMP_SpriteAsset Asset()
        {
            ...
            if (_asset == null)
            {
                MyValheimModPlugin.Log.LogWarning(
