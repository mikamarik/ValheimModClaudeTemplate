...
"""Turns the frames the readme_gifs autotest recorded into the README's GIFs.
    ...
    ./scripts/autotest.sh readme_gifs       record the frames into .devtest/gifs/<name>/ (AutoTest.Gifs.cs)
    python3 scripts/make-gifs.py            make docs/media/<name>.gif for every clip in CLIPS
    python3 scripts/make-gifs.py example    make only that one

A take the recorder refused (the scene was not as the clip needs it) can leave bad.txt next to
its frames, with the reason; this script then stops instead of making the GIF.
...
# name: width in pixels, frames a second (the recording's, AutoTest.GifFps), colours, the change
# that counts: a block is drawn again when one pixel changed by more than `threshold`, or the block
# on average by more than `mean` (a dark panel fading off dark ground changes every pixel a little;
# without `mean` its blocks would stay on the ground for good). `stats` is how the palette is
# picked: "full" from every pixel, "diff" mostly from what moves (a short red flash stays red).
# ValheimTomrer's tuned values: a UI clip width 960, threshold 10, stats "diff"; a world clip with
# building width 880, threshold 32, mean 10; a HUD overlay on the world width 960, threshold 14.
CLIPS = {
    "example": dict(width=960, fps=15, colors=256, threshold=10, mean=None, stats="diff"),
}
