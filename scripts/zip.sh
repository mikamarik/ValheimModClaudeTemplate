#!/usr/bin/env bash
# Builds the Release DLL and packs the Thunderstore zip: thunderstore/build/<Name>.zip.
...
dotnet build "$CSPROJ" -c Release -v minimal -nologo -p:DeployToGame=false
...
grep -q "## $MANIFEST_VERSION" "$REPO/CHANGELOG.md" || fail "CHANGELOG.md has no '## $MANIFEST_VERSION' block"

SIZE=$(sips -g pixelWidth -g pixelHeight "$ICON" | awk '/pixel/ { printf "%s ", $2 }')
[[ "$SIZE" == "256 256 " ]] || fail "icon.png is ${SIZE% }, it must be 256 256"

# A Release build leaves the autotest out (src/Dev is behind #if DEBUG).
